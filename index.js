...
function getArtistByIndex(array, number) {
  /*Your Code Here*/
  let string = `the artist at index ${number} is ${array[number].name}`
  return string;
}
...
function get20s(array) {
  ...
  for (let i = 0; i < array.length; i++)
  {
    ...
    let x = (array[i].years.split(' ')[0])
    let y = (array[i].years.split(' ')[2])

    if (x - 1900 >= 0 && x - 1900 <= 99 && y - 1900 >= 0 && y - 1900 <= 99) {
      blankArray.push(array[i].name);
    }
  }
  return blankArray;
...
function addArtist(array) {
  ...
  bio: 'Born and raised in NYC. Learning to code!'
  }
  array.push(myInfo);
  return array;
}
